...
exports.createDBs = async function (cloudant, dbNames) {
    try {
...
exports.destroyDBs = async function (cloudant) {
    try {
...
exports.stressCloudant = async function stressCloudant(cloudant, dbName, type, num) {
    try {
        let DB = await cloudant.db.use(dbName);
        let iters = [...Array(num).keys()];
        if (type === 'queries') {
            await Promise.all(iters.map(() => DB.list({ include_docs: true })));
        }
        else if (type === 'writes') {
            let res = await Promise.all(iters.map(() => DB.insert({ hello: 'hello' })));
        }
        else if (type === 'lookups') {
            let id = Math.random() * 1000 + 'xyz';
            DB.insert({ _id: id, hello: 'hello' });
            await Promise.all(iters.map(() => DB.get(id)));
        }
        else if (type === 'all') {
            for (let i = 0; i < 3; i++) {
